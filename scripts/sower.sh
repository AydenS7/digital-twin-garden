#!/bin/zsh
# Sower — once a day, deep-reads the vault and writes the daily 5-item note to Daily/YYYY-MM-DD.md
#
...
cd "$VAULT" || exit 1
"$CLAUDE_BIN" -p \
  "You are the Sower. Read Meta/Sower.md and produce today's note now, following it exactly. Today is $(date '+%Y-%m-%d')." \
